import { OLD_TESTAMENT_BOOKS } from "@/consts/global";
...
// Figure out if a book is Old Testament or New Testament
function determineTestament(book: string): "OT" | "NT" {
  return OLD_TESTAMENT_BOOKS.includes(book.toLowerCase()) ? "OT" : "NT";
}

// Search for verses in the KJV Bible using semantic similarity
async function searchKJVVerses(
  query: string,
  topK: number = 5
): Promise<any[]> {
  try {
...
// Find the Hebrew/Greek text for a specific verse
async function findOriginalLanguageVerse(
  book: string,
  chapter: number,
  verse: number
): Promise<any> {
  try {
    ...
    if (!indexName) {
      console.error(
        `Missing environment variable: PINECONE_INDEX_${language.toUpperCase()}_NAME`
      );
      return null;
    }

    // Look for verse by exact ID first
    const verseId = `${testament}-${book
      .toLowerCase()
      .replace(/\s+/g, "-")}-${chapter}-${verse}`;
    let originalVerse = await fetchVectorById(indexName, verseId);

    if (!originalVerse) {
      // Fallback: search by embedding similarity
    ...
    console.error(
      `Error finding original language verse for ${book} ${chapter}:${verse}:`,
      error
    );
    return null;
  }
...
// Main function: Search verses and provide analysis with original languages
export async function analyzeVerses(
  query: string,
  topK: number = 5
): Promise<AnalysisResult> {
  try {
    // Step 1: Find relevant KJV verses using AI similarity
    ...
    // Step 2: Get Hebrew/Greek text for each verse
    console.log(
      `📖 Looking up original language texts for ${kjvResults.length} verses`
    );
    ...
    // Step 3: Use AI to create scholarly analysis
    console.log(
      "🤖 Generating verse explanation with original language context"
    );
...
// Use AI to generate scholarly explanation of verses with original language context
async function generateExplanation(
  query: string,
  verses: VerseResult[]
): Promise<string> {
  try {
    // Format verses for AI prompt (KJV + Hebrew/Greek)
    const versesContext = verses
      .map((verse) => {
        ...
        if (verse.originalText) {
          const langName =
            verse.originalLanguage === "hebrew" ? "Hebrew" : "Greek";
...
Keep your explanation engaging, educational, and accessible to general readers. Focus on how the original language enhances our understanding of the passage.`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a knowledgeable Bible scholar who explains scripture with attention to original languages, historical context, and practical application.",
        },
        {
          role: "user",
          content: prompt,
    ...
    return `Here are the relevant verses for "${query}":\n\n${verses
      .map((v) => `${v.book} ${v.chapter}:${v.verse} - ${v.kjvText}`)
      .join("\n\n")}`;
  }
}

// Helper function for specific verse lookup
// Analyze one specific verse (like "John 3:16")
export async function analyzeSpecificVerse(
  book: string,
  chapter: number,
  verse: number
): Promise<AnalysisResult> {
  try {
    ...
    console.error(
      `Error analyzing specific verse ${book} ${chapter}:${verse}:`,
      error
    );
    throw error;
  }
